...
\begin{enumerate}
    \item nuXmv does not guarantee stable behavior on \minrv{Ecall} instructions; this introduces a sense of non-determinism where nuXmv considers more programs than could actually be run on the MINRV8 architecture.
...
This leaves us with good reasons to assume that the set of programs nuXmv quantifies over when running proofs is sufficiently close to the set of \enquote{real} programs of the MINRV8 architecture including jumps and branches.
