...
\todo{mention that other fields are well modelled}
...
\todo{gls rop}
\todo{introduce ROP}
In practice though, jumps and branches usually are security relevant since they are targeted by adversarial programs trying to manipulate jump addresses to launch e.g. ROP attacks.
...
Additionally, the implementation might not be able to model programs including jumps and branches but the set of programs considered by nuXmv does include those where jumps have been linearized\footnote{%
...
\todo{Mention these goals in introduction/background}
...
It shall now be subject to investigate whether this work is touched by aforementioned two issues.
...
You as a reader might have wondered, why we decided to concretely model all flows of information through the model of the MINRV8 architecture and indeed: In scope of this thesis, the actual content of registers is not mandatory.
...
Another issue, a reader might have stumbled upon is that in section \ref{sec:results} it was mentioned that counter-examples never involved flows of information where the bit-wise labels of values in the architecture was mandatory.
