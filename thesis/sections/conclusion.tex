...
% TODO: Replace "requirements" with "objectives"
...
As a first step we developed the MINRV8 architecture in section \ref{sec:minrv8} that was inspired by the \gls{riscv} architecture and implemented in \gls{nuxmv}.
...
In the introduction, we subjected this work to three other goals: it was claimed that the approach presented as part of this thesis is \textit{viable}, i.e. feasible, realistic and generalizable, \textit{relevant}, i.e. it is able to detect issues, and \textit{supplemental}, i.e. it enhances on related work.
...
\begin{description}
    ...
    \item[MMU] In light of \cite{KhakpourSD13} a model of an MMU could be implemented to make use of information flow tracking on user-mode level.
    ...
    For example, there are machine-readable versions of the \gls{arm} architecture; both \cite{Reid17,Fox02} either used or developed machine-readable specifications that might be used for this endeavor.
