...
As you can see, each of these four labels includes labels from two domains of information flow control.
...
The second of these domains is the one of \textit{integrity}, i.e. are the values we deal with integrous or not malicious?
Each of the labels states that a piece of information is either trusted (\T{}) or untrusted (\U{}).

These labels come with a $ \sqcup $ operation as well as an implicit $ \preceq $ partial order on them which can be derived from $ \sqcup $.
This partial order is depicted in figure \ref{fig:sec-lattice} as a hasse diagram.
The intuition behind the ordering is that information is always allowed to flow up while it is not allowed to flown down.
...
int b : PU = c;
    ...
    Then, another variable labelled with \PU{} is initialized and assigned to value of \aslinline{a}.
    This flow of information would be permitted by the lattice of $ \Sigma $ as information that is labelled with \PT{} flows to a destination that is labelled with \PU{} which is $ \preceq $-greater than the value.

    If this flow of information was to occur the other way round, however, it would have been prohibited.
\end{example}
...
However, there are more complex operations in SecVerilogBL than simply assigned values to variables.
This is why the authors in \cite{Ferraiuolo17} introduce type system rules to define how information flows throughout the system, i.e. how operations generate new security labelings in the form of types.
...
In the context of type systems $ A $, $ B $ and $ C $ often are so called \textcquote{Ferraiuolo17}{typing judgements} where some \textit{environment} type some \textit{expression}, e.g. if $ X $ is a type environment, $ x $ an expression and $ \tau $ a type one possible typing judgement would be $ X \vdash x : \tau $ which means that $ x $ is of type $ \tau $ under the environment $ X $.
