...
\begin{itemize}
    ...
    This is only given if both of the sources of an operation are not controlled by an attacker, i.e. trustworthy.
\end{itemize}
...
This allows us now to add security label tracking to our model whereby we will will be track the confidentiality and the integrity of labels values.
...
The values themselves change based on the current instruction and the information flow tracking semantics as described in \ref{sec:ifc-model}.
