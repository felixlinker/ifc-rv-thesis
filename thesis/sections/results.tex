...
The latter aspect of validity is guaranteed by one simple constraint that is depicted in the following snippet:
\begin{lstlisting}[
    language=smv,
...
\end{lstlisting}
This ensures that the interrupt handling mechanisms are set up correctly and is mandated by the \gls{riscv} specification (cf. section \ref{sec:rv-exn} and figure \ref{fig:interrupt-handling}).
...
The first assumption to be introduced is called \smv{CLR_ON_RET} and mandates from machine-mode to only return control to user-mode when the registers are cleared of any confidential data.
