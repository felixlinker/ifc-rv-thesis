...
\todo[inline]{Klarstellen, dass keine Schwachstelle gefunden wurde}
...
\todo{Wurden arch.-ref. eingeführt?}
Since only assumptions and no architectural refinements were introduced to the model, it was also tested how the results would change when deliberate breaking points are introduced to the architecture.
These breaking points mimic known vulnerabilities to real-world architectures and will therefore also give an overview of the capabilities of the verification approach of this thesis.
...
\todo{Refernce to threat model and example of direct passing of information}
...
However, since \lstinline{SYSCALL} does not set the stack-pointer accordingly, it is the obligation of the interrupt handler to set the stack-pointer back to user-mode\footnote{%
