...
This section is grouped as follows: section \ref{sec:assumptions} will introduce the aforementioned assumptions and \smv{INIT} constraints.
...
Assumptions, on the other hand, are not part of the model in a technical sense but mark a result of the verification of the architecture: Each assumption constitutes a rule, e.g. for \gls{os} or compiler engineers to obey to write or generate non-vulnerable code.
...
Secondly, two more groups of constraints ensure the initial state to comply with the specification is guaranteed.
...
A checkmark denotes that the respective assumption is critical for the respective property, i.e. there exists a counter-example proving the property to be false if the assumption at hand is not assumed.
...
It is noteworthy that these two groups of assumptions can be characterized by the way they constrain information flow which aligns perfectly with the intuition of the two types of labels:
...
By default, the MINRV8 architecture is not vulnerable to these attacks.
However, making the architecture vulnerable will put the approach of this thesis to formally verify an instruction set architecture to a test.
...
\begin{enumerate}
    ...
    Trigger a System Management Interrupt to transfer control to System Management Mode.
...
One of these routines to be left out was changing the stack-pointer by hardware.
...
In the case of the MINRV8 architecture, no such special mechanisms are given.
Therefore, the vulnerability can only be triggered by an external interrupt but this does not conflict with the core idea of the SYSRET vulnerability.
...
This ensures that machine-mode does not deliberately decide to use a bad stack-pointer.
...
One approach lies upon the burden of ensuring security on the programs running on the architecture and is expressed in the form of an assumption, the other modifies the architecture and does not rely on programs to implement certain guidelines.
...
We therefore make the aforementioned assumptions to show that \textit{given} such mechanisms, the herein proposed hardware mitigation to the SYSRET vulnerability can be verified by our approach.
...
This windows is highlighted as the area coloured in yellow.
...
If any of these are not assumed, the inputs to memory or \gls{csr} related instructions might become low, e.g. somewhere in the orange-coloured area of the second row.
...
This window is coloured in green.
...
Again, the orange-coloured area of the first row shows where user-mode might be able to load secrets from memory or cache if these assumptions were not in place.
