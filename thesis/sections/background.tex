...
\begin{example}
    ...
    The first of these domains is the one of \textit{confidentiality} - each label states that a piece of information is either public (\P{}) or confidential (\C{}).
    ...
    Each label states that a piece of information is either trusted (\T{}) or untrusted (\U{}).
...
\begin{example}
    ...
    If the check was omitted and \lstinline{ns == 1}, \lstinline{read} would be written with \lstinline{mem[read_addr]} when \lstinline{reg_ns[read_addr] == 0}, a variable of type \lstinline{PU} would be written with a value of type \lstinline{CT}.
...
The input language supports models that are very conservative in regard to the model of computation that is taken as a basis.
