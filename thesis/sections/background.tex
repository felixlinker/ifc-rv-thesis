...
\begin{figure}
    \centering
    \begin{tikzpicture}[node distance=1.2cm and 0.2cm,auto]
        \tikzstyle{block} = [
            draw=black,
            rectangle,
            minimum width=2.4cm,
        ...
        \tikzstyle{vecarrow} = [
            ...
            decoration={
                ...
                mark=at position 1 with {
                    \arrow[semithick]{open triangle 90}
                }
            },
        ...
        \node[block] (hardware) {Hardware};
        \node[block] (compilers) [above=of hardware] {Compilers};
        \draw[vecarrow] (compilers) to (hardware);

        \node[block] (os) [above right=of compilers] {OS};
        \node[block] (programs) [above left=of compilers] {Programs};

        \begin{scope}[on background layer]
            \draw[vecarrow] (os.center) to (compilers);
            \draw[vecarrow] (programs.center) to (compilers);
        \end{scope}

        \coordinate (yMid) at ($(programs)!0.5!(os)$);
        \coordinate (mid1) at ($(yMid)!0.5!(compilers)$);
        \coordinate (right1) at (os.east |- mid1);
        \coordinate (mid2) at ($(compilers)!0.5!(hardware)$);
        \coordinate (right2) at (os.east |- mid2);
        \draw[dashed] (programs.west |- mid1) -- (right1);
        \draw[dashed] (programs.west |- mid2) -- (right2);

        \node[draw=none] (pl) [right=0cm of right1] {PLs};
        \node[draw=none] (isas) [right=0cm of right2] {ISAs};
    \end{tikzpicture}
...
In \cite{Zhang15, Mukherjee16} attempts to verify hardware in general have been proposed and implementations of processors have been verified against higher level properties in \cite{Beatty94, Berezin98, Trippel19} or against their actual specifications in form of \glspl{isa} in \cite{Burch94, Reid16}.
...
\gls{nuxmv} supports both finite and infinite state transition systems.
...
In the context of \gls{nuxmv}, an invariant is formally given by a next-expression, i.e. an expression that can contain the use of the \smv{next} function.
...
When an \gls{ltl} specification \smv{p} is checked by \gls{nuxmv}, it is verified that \smv{p} is true for \textit{all} infinitely long execution paths.
...
\begin{figure}
    \centering
    \begin{tikzpicture}[
        level/.style={sibling distance=30mm/#1,level distance=10mm},
        vertex/.style={draw=black,circle,minimum height=2mm,minimum width=2mm},
    ]
        \node[vertex] {$ x_0 $}
        child {
            node[vertex] {$ x_1 $}
    ...
    \end{tikzpicture}
    \caption{\gls{ctl} Interpretation Example}
    \label{fig:ctl-int}
\end{figure}
...
\begin{example}
    ...
    In \gls{nuxmv}, specifications are introduced by one of \smv{INVARSPEC}, \smv{LTLSPEC} or \smv{LTLSPEC} and optionally may be preceded by a name which is omitted in this example.
...
\gls{nuxmv} supports a plethora of techniques to check specifications, e.g. the \citetitle{nuXmv} \cite{nuXmv} lists 47 commands to check a specification.
% TODO: \gls{ifc}
