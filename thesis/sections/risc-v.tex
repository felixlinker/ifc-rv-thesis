...
Most of these instructions are implemented in two versions: a register version (or R-type instruction) and an immediate version (or I-type instruction).
...
If this address is given as an argument to a respective load instruction, it will be aligned to the address 0x0.
...
These instructions will be inspected in more detail in section \ref{sec:rv-priv-arch}.
...
In this section, the aforementioned concepts will be introduced subsequently while the respective parts of the \gls{mstatus} register will be discussed whenever applicable.
...
To keep things simple, the focus of this thesis lies in secure embedded systems\footnote{%
...
These permissions are set up in the \gls{pmpcfg} registers of which there are four.
...
If MPRV is set to one, \textcquote{RiscVISAP}{load and store memory addresses are translated and protected as though the current privilege mode were set to MPP}, i.e. they're performed with the privilege level that was preempted by taking the trap.

Additionally, the \gls{mstatus} register supports the MXR field that allows reading executable memory regions.
...
\begin{displaycquote}{RiscVISA}
    We use the term \textit{exception} to refer to an unusual condition occurring at runtime associated with an instruction in the current RISC-V thread.
...
\gls{xepc} holds the address of the original instruction stream that was preempted, i.e. either the address of the instruction that caused the exception or the instruction that was next to be executed as an external interrupt was taken.
...
On execution of the $x$RET instruction, the architecture will reset the state according to the values stacked earlier, i.e. \gls{xstatus}.$x$IE is written with \gls{xstatus}.$x$PIE, the privilege mode is set to \gls{xstatus}.$x$PP, the \gls{pc} is set to \gls{xepc}, and normal execution continues until the next interrupt or exception occurs.
...
However, these mechanisms are beyond this thesis's scope as they \begin{enumerate*}[label=\alph*)]
    \item often do not introduce new concepts to the architecture as a whole but build upon already introduced ones\footnote{%
...
To summarize:
\glspl{pma} and \gls{pmp} entries make general properties of physical memory transparent to software, allowing it to adapt to possible interactions with respective regions correctly.
...
\begin{enumerate}
    ...
    \item Take the trap by jumping to the trap base vector and, if an interrupt is being serviced, take vectoring into account.
    \item Return from trap handler.
\end{enumerate}

The differences between handling exceptions and interrupts can be summarized by the following:
\begin{itemize}
    \item Exceptions cannot be disabled therefore they will always be handled
...
Therefore, \glspl{pma} will be given in the form of cacheability settings but nothing more.
...
The reasoning behind the architecture and features to include was given in section \ref{sec:risc-v-selection}.
...
Machine words are generally interpreted as signed-words.
Thus, there will not be unsigned counterparts to integer arithmetic or the comparison instruction.
...
\begin{table}
    ...
    \begin{tabular}{|l p{8cm}|}
        ...
        \minrv{Sll rd, rs1, rs2} & Set register \minrv{rd} to the content of register \minrv{rs1} shifted left logically (i.e. without sign-extension) by the value in register \minrv{rs2} \\
...
Therefore, in the context of MINRV8, \gls{mstatus} has 4 bits comprising two stacking bits for taking traps, i.e. MPP (machine-mode prior privilege) and MPIE (machine-mode prior interrupts enabled), a MEIP bit to signal pending external interrupts and an MIE bit to en- or disable interrupts globally.
...
\begin{displaycquote}[p.11-7]{IntelSystemProgramming}
    ...
    \begin{description}
        ...
        Write-back operations are triggered when a cache lines need to be deallocated, such as when new cache lines are being allocated in a cache that is already full. \textelp{}
...
The development of the MINRV8 architecture was tightly coupled to its implementation in nuXmv, which will be presented in section \ref{sec:model-implementation}.
...
In each of these sections, the variables and their corresponding types will be stated declared as part of the modules after which the transition relation for each of the respective variables will be outlined.
...
All these aspects are not relevant to the goal of this thesis.
...
There is no input variable for an immediate value which might be needed by the \minrv{Loadi} instruction because otherwise, we would waste extra state space.
...
nuXmv allows constraining the transition of variables by arbitrary formulas expressed in propositional logic.
...
Firstly, whenever there is an exception - be it an exception triggered by a pending external interrupt or an environment call to machine-mode exception - the \gls{hart} switches to machine-mode and does not execute the current instruction, i.e. the value of \smv{op}, \smv{rd}, \smv{rs1} and \smv{rs2} are ignored.
...
Therefore, if the cache holds some not persisted values of write-back-cacheable memory regions and either the cache's target changes or the \gls{pmacfg} attributes change such that the cache is not write-back-cacheable anymore, the content of the cache will be written to memory as well.
...
It was shown that this list of instructions is complete in comparison to RISC-V minus very few but negligible instructions that were not included and instructions made redundant by design decisions or that can be emulated.
...
The implementation was conducted by hand with the aid of the preprocessor engine pyexpander \cite{pyexpander}.
