...
% TODO: Add motiviation/goals of RISC-V
...
An example for a naming string is the very common subset of the \gls{isa} RV64IMAFD which officially is abbreviated by RV64G.
...
We chose to go with the \gls{riscv} architecture for the approach of this thesis because of its high customizability.
...
The base integer instruction set only offers some core functionality that can easily be understood and modeled while it is not artificially or overly simplified thus we hope that our findings are relevant to and applicable for other real-world architectures.
...
Each \gls{hart} has its own registers including a \gls{pc} and its own independent instruction fetch unit.
...
The base integer instruction set supports a couple of instructions for $ + $ and $ - $ and integer comparisons as well as the boolean operators $ \land $, $ \lor $ and $ \oplus $.
...
Firstly, simple load and store instructions that or write to memory.
...
Load and store operations in \gls{riscv} can read or write words of size 8-, 16-, 32-, 64- or 128-bits, depending on the maximum word length of the base instruction set of choice.
Therefore, memory in \gls{riscv} is byte-addressable.
...
We chose to not model these registers as they don't contain any information that either is relevant to this thesis or couldn't be modeled in another way.
...
\begin{table}
    ...
    \begin{tabular}{| r | p{8cm} |}
        ...
        \textbf{\acrshort{mvendorid}} & Stores an ID of the manufacturer of the processor \\
...
We will subsequently give an introduction to these registers by guiding the reader through the control flow on dealing with interrupts which is depicted in figure \ref{fig:interrupt-handling}.
...
\begin{figure}
    ...
    \begin{tikzpicture}[
        ...
        \node[box] (pending) {
            An interrupt $ i $ is pending in mode $ y $\\
            Let $ x := \text{machine-mode} $ \\
            Let $ a := \text{current address} $
        };

        \node[draw,circle] (takePre) [below=of pending] {};
        \draw[->] (pending) to (takePre);

        \node[box] (deleg) [left=of takePre] {$ x := x - 1 $};
        \draw[->,bend right] (takePre) to node[above,xshift=-0.8cm] {
            if $ x\text{ideleg}[i] \land x > y $?
        } (deleg);
        \draw[->,bend right] (deleg) to (takePre);

        \node[box] (take) [below=of takePre] {
            ...
            $ x\text{cause}[31] := 1 $\\
            $ x\text{cause}[30..0] := i $\\
            Write $ x\text{tval} $ accordingly\\
            Let $ t := x\text{tvec} $
        };
        \draw[->] (takePre) to node[right] {
            else if $ (x = y \land x\text{status}.x\text{IE} \land x\text{ie}[i]) \lor x > y $?
        } (take);

        \node[draw,circle] (jumpPre) [below=of take] {};
        \draw[->] (take) to (jumpPre);

        \node[box] (vectored) [right=of jumpPre] {$t := t + 4 \times i $};
        \draw[->,bend left] (jumpPre) to node[above,xshift=0.4cm] {if $ x\text{tvec.MODE} $?} (vectored);
        \draw[->,bend left] (vectored) to (jumpPre);

        \node[box] (handling) [below=of jumpPre] {Jump to $ t $ and execute instructions};
        \draw[->] (jumpPre) to (handling);

        \node[box] (return) [below=of handling] {
            $ x\text{status}.x\text{IE} := x\text{status}.x\text{PIE} $\\
            Set privilege mode to $ x\text{status}.x\text{PP} $\\
            Jump to $ x\text{epc} $
        };
        \draw[->] (handling) to node[right] {on $ x\text{RET} $} (return);
    \end{tikzpicture}
...
If an interrupt is set pending via \gls{mip} by default it will be taken by machine-mode if the corresponding bit in \gls{mie} is set and interrupts are enabled globally.
...
If a pending interrupt is taken, the \gls{mepc}, \gls{mcause} and \gls{mtval} are written.
...
% TODO: is page-based virutal memory given?
